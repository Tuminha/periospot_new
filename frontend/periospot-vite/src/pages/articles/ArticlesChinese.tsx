...
const ArticlesChinese = () => {
  const [searchQuery, setSearchQuery] = useState("");
  const [selectedCategory, setSelectedCategory] = useState(ALL_CATEGORY);
  const [sortBy, setSortBy] = useState("latest");
  const [currentPage, setCurrentPage] = useState(1);

  const posts = useMemo(() => getPostsByLanguage("zh"), []);
        ...
        <div className="container mx-auto px-6">
          ...
          >
            <div className="inline-flex items-center gap-2 mb-4">
              ...
              <span className="text-sm font-medium text-primary">中文文章</span>
            </div>
            <h1 className="font-display text-4xl md:text-5xl font-semibold text-foreground mb-4">
              博客
            </h1>
            <p className="text-muted-foreground text-lg max-w-2xl mx-auto">
              专家见解、临床案例以及牙周病学和种植学的最新研究。
            </p>
          </motion.div>
          ...
          >
            显示 {paginatedPosts.length} 篇，共 {filteredPosts.length} 篇文章
          </motion.p>
          ...
          {paginatedPosts.length > 0 ? (
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
              {paginatedPosts.map((post, index) => (
                ...
                >
                  <Link to={`/articles/chinese/${post.slug}`} className="block">
                    ...
                    <div className="p-5">
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <span className="text-xs text-muted-foreground">
                          {formatReadTime(post.readTimeMinutes, "zh")}
                        </span>
                      </div>
                    </div>
                  </Link>
            ...
            >
              <p className="text-muted-foreground text-lg">没有找到符合条件的文章。</p>
              <button
                onClick={() => {
                  setSearchQuery("");
...
export default ArticlesChinese;
